...
| 用户说的话 | 走哪条流程 |
|---|---|
| "帮我准备面试" / "我要建故事库" / 给一段经历 | **A. 挖掘新故事** → `prompts/story-mining.md` |
| 贴出一道具体 BQ（"Tell me about a time…"） | **B. 回答一道题**（先查库，下方流程） |
| "我这个故事讲得好吗" / 贴出已有答案 | **C. 打磨已有故事** → `prompts/structuring.md` |
| "模拟面试" / "出几道题考我" | **D. 模拟面试**（v1 轻量版，下方） |
| 给了 **JD + 简历** / "针对这个岗位帮我准备 BQ" / "这家会问什么、我怎么答" | **E. JD 驱动的 BQ 选题 + 准备** → `prompts/jd-driven-prep.md` |
...
## 流程 E — JD 驱动的 BQ 选题 + 准备（与 job-description-skill 挂钩）
...
2. **拿简历全文** — 给每段经历打能力标签，为配对做准备。简历太薄 → 转流程 A 先挖几个故事。
...
5. **缺口清单** — JD 要、简历没素材的题如实列出，导流到流程 A 现场挖。
6. **HTML 报告** — 默认输出到 `~/Desktop/Claude skills/bq-prep-<company>-<role>-<YYYYMM>.html`（视觉规范见 `assets/bq-prep-report.md`）。
7. **回写** — 把 JD↔故事映射写回 job-description-skill 的 jd-bank 文件「Predicted Questions」，形成 Career Copilot 闭环。

> 这是 job-description-skill 流程 4（Interview Predictor）的下游深化：那边给「会问什么」，这里给「**用你的真实经历怎么答 + 模板**」。
...
- `_index.md` 是反查表：**能力标签 → 故事文件**，是流程 B 复用的入口。
...
- `prompts/story-mining.md` — 四层追问引擎（流程 A 核心）
- `prompts/structuring.md` — 结构化诊断与改写（流程 C）
- `prompts/jd-driven-prep.md` — JD × 简历 → Top 20 选题 + 准备模板（流程 E，挂钩 job-description-skill）
- `assets/bq-prep-report.md` — 流程 E 的 HTML 报告视觉规范
